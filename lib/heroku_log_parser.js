...
var LogParser = function (model) {
  ...
  // chain the events downstream
  ['newservice', 'statechangeservice', 'newsource', 'updatesource'].map(function(item){
    self._heventCalls.events.on(item, self._rebindEvent.bind(self, item))
  })
...
LogParser.prototype._rebindEvent = function(event, data){
